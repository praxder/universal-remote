...
- **Exact `pyatv` entry points for the installed version**: `scan(loop, hosts=[…])`, `pair(config, Protocol.Companion, loop)` → `begin()/device_provides_pin/pin()/finish()/has_paired/service.credentials/close()`, `connect(config, loop)`. Nav/select/menu/home map to `atv.remote_control` (`up/down/left/right/select/menu/home`); **volume maps to `atv.audio.volume_up()/volume_down()`** since `RemoteControl.volume_*` is deprecated in favour of the `Audio` interface; text uses the keyboard interface (`text_set`/`text_append`). Confirm against the pinned version before finalising the mapping (mirrors how the LG change confirmed `aiowebostv` 0.7.5 entry points during task 1.2).
- **Volume routing**: whether `Audio.volume_up`/`volume_down` reach the TV directly or via HDMI-CEC on the user's setup — declared supported, accepted best-effort. `pyatv` exposes no mute on either `RemoteControl` or `Audio` (confirmed against source), so MUTE is correctly omitted from the capability set.
