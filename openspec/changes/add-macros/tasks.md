...
- [ ] 1.1 Rename `ScriptResult` to `ActionResult` in `tui/actions.py` and update every
...
- [ ] 1.2 Add a frozen `ActionContext` dataclass to `tui/actions.py` carrying
      `remote_ip`, `session`, `notify`, `macros`, `custom_buttons`, `device_id`, and
      `platform`.
- [ ] 1.3 Write a test asserting `run_action` passes an `ActionContext` to the catalog
...
- [ ] 1.4 Update `RemoteScreen._execute` to build an `ActionContext` and pass it to
...
- [ ] 2.1 Create `src/universal_remote/macros/models.py` with a `Macro` (stable `id`,
      `name`, ordered `steps`) and the four step shapes from design decision 7 —
      `key`, `text`, `action`, `pause` — plus `to_dict`/`from_dict` following
      `devices/models.py`.
- [ ] 2.2 Write tests for a step's human-readable description (`Key: HOME`,
...
- [ ] 2.3 Write tests for the registry: add, get by id, delete, list in saved order, and
      ...
      `macros/registry.py`.
- [ ] 2.4 Write tests for draft operations — move a step up, move it down, no-op at the
...
- [ ] 3.1 Add `macros: dict` to `Preferences`, a fault-tolerant branch in
...
- [ ] 3.2 Add `self.macros: dict = {}` to `UniversalRemoteApp.__init__` and populate it
      from the loaded preferences in `on_mount`, alongside `custom_buttons`.
- [ ] 3.3 **Add `macros=self.macros` to the `Preferences(...)` call in
      ...
      on any theme change.
- [ ] 3.4 Test that theme, shortcuts, custom buttons, and macros all survive together
      across a restart, mirroring the existing "actions coexist" scenario.

## 4. Recording on the remote

- [ ] 4.1 Write a test that the top row shows a fourth `Macros` button to the right of
      Back, then add it to `RemoteScreen.compose`.
- [ ] 4.2 Write a test that entering the recording state does not make the remote scroll
      ...
      mode). No new rows.
- [ ] 4.3 Render the indicator's cancel hint from
      `display_label(effective_key("global.go_back", overrides))`. Test that rebinding
      Go Back changes the hint text.
- [ ] 4.4 Write tests for capture at each tap point, then implement: a `key` step
...
- [ ] 4.5 Write a test that activating a `run_macro` custom button while recording runs
...
- [ ] 4.6 Write a test that the Go Back key while recording cancels the recording and
      leaves the session connected — it must not close the remote — then branch
      `action_go_back` on the recording state.

## 5. Macros list modal

- [ ] 5.1 Write tests for the list modal: saved macros listed by name in saved order; a
...
- [ ] 5.2 Write a test that Create Macro **dismisses** the modal (does not leave it on
...
- [ ] 5.3 Write tests for the return paths, then implement: `■ Stop` saves a new macro
...
- [ ] 5.4 Write a test that Enter on a macro row opens its detail modal, then wire it.

## 6. Macro detail modal

- [ ] 6.1 Write tests for the draft: the modal renders from an in-memory draft, not the
...
- [ ] 6.2 Lay out the controls as two rows of buttons with `min-width: 0` and explicit
...
- [ ] 6.3 Write tests for the step controls — move up, move down, no-op at the ends,
...
- [ ] 6.4 Write tests for add-step, then implement: it dismisses the detail modal, puts
...
- [ ] 6.5 Write a test that Go Back or `■ Cancel` during capture-one returns to the
      detail modal with the draft unchanged, then implement it.

## 7. Pause steps

- [ ] 7.1 Write tests for the pause prompt: a valid millisecond value inserts a pause
...
- [ ] 7.2 Write a test that opening an existing pause step reopens the prompt prefilled
      with its current value and that a new value replaces it, then implement it.

## 8. Playback

- [ ] 8.1 Write tests for the playback modal: it names the macro, reports progress
...
- [ ] 9.1 Write tests for the macro picker: it lists saved macros by name; confirming
...
- [ ] 9.2 Write a test that reopening the configuration for a button that already has a
      Run Macro action preselects the referenced macro, then implement the prefill.
- [ ] 9.3 Write a test that renaming or editing an assigned macro changes what the button
      does (proving the action holds an id, not a copy).

## 10. Documentation and preflight

- [ ] 10.1 Document macros in the user-facing docs: recording, editing, assigning one to
      a custom button, and the deliberate limitations (no nesting, no loops, snapshotted
      button steps, no undo of a partial run).
- [ ] 10.2 Update `tests/test_shortcuts_catalog.py` for the Remote action count (thirty-one
...
- [ ] 10.3 Preflight: run the formatter, fix all lint findings, and run the full suite.
