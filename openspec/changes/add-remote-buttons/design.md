...
**As built:** the compact rules live in `RemoteScreen.DEFAULT_CSS` (a `Screen`'s inline styles are `DEFAULT_CSS`, not `CSS` — the latter is silently ignored). The one-row button rule is `#remote Button { … border: none }`, id-scoped on purpose: Textual sets Button borders per-variant (`Button.-default`), and that class specificity beats a plain `RemoteScreen Button`, so `border: none` only wins from an id-scoped selector — without it, border-box sizing collapses the single row and every button renders blank. The `#remote` container plus its row containers are set to `height: auto`. Auto-height matters for the fit guarantee: a filling (`1fr`) container clips overflow silently, so the fit test cannot see it; with `height: auto` an over-tall button set makes the *screen* scroll — a visible, asserted signal (`test_..._does_not_scroll` checks `screen.max_scroll_y == 0`). The disabled dim uses `text-opacity: 40% !important` (to beat Textual's built-in 0.6 disabled opacity) plus a `$panel` background as a second, border-independent cue.
