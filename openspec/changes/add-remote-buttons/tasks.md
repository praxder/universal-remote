...
- [x] 4.1 Confirm the change's spec deltas (`specs/remote-control-core/spec.md`, `specs/tui-remote/spec.md`) match what was built; adjust wording if the implementation diverged. _(Reviewed: vocabulary, surface button set, digit bindings, and scan icons match. The layout was later revised per feedback — buttons are bordered/bigger and centered like a physical remote (D-pad cross), which trades the strict 80×24 fit for a taller baseline that scrolls on short terminals; the `tui-remote` surface requirement + scenarios and the design decision were updated to match.)_
