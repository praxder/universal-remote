...
- **Exact `rokuecp` entry points for the installed version**: the `Roku` client constructor and how the `aiohttp` session is supplied, the button-press method and its accepted key vocabulary, the literal-text method, and the reachability/device-info method plus its error types. Confirm against the pinned version before finalising the key map (task 1.2).
  - **Resolved (task 1.2, `rokuecp==0.19.5`):** `Roku(host, ..., session=<aiohttp.ClientSession>)` — the adapter injects and owns the `aiohttp` session (passing one keeps `_close_session` False, so `close_session()` is a no-op and `_release` closes the aiohttp session directly). Button press is `remote(key)`; literal text is `literal(text)`; reachability is `update()`. `remote()` **lower-cases its argument and looks it up in `const.VALID_REMOTE_KEYS`**, whose *keys* are snake_case — so the accepted vocabulary is `up`/`down`/`left`/`right`/`select`/`back`/`home`/`volume_up`/`volume_down`/`volume_mute`/`channel_up`/`channel_down`/`play`/`reverse`/`forward`, **not** the ECP output strings (`Rev`/`Fwd`/`VolumeUp`/…) guessed above (those would raise `RokuError`). Errors derive from `RokuError` (base), with `RokuConnectionError` and `RokuConnectionTimeoutError` subclasses — a single `except RokuError` covers unreachable, refused, timeout, and bad-response.
