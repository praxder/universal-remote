...
- **Run a configured button on click.** A custom button that has an action RUNS it on click; a button with no action still opens config (the Phase-1 rule "click runs iff the button has an action" now has its left branch). Re-editing a configured-with-action button uses a distinct edit gesture: press an edit-mode key, then activate the button (click or shortcut) to open its config instead of running it (a modifier-click was ruled out — Cmd+Click is undetectable in a terminal; see design).
