...
The system SHALL define a platform-agnostic set of remote keys that all callers use, independent of any TV brand. The vocabulary MUST cover directional navigation (up, down, left, right), select/OK, back, home, volume up, volume down, mute, and power.
...
An adapter SHALL declare which keys it supports and whether it supports text entry and remote power-on. Callers MUST be able to read these capabilities without connecting to a device.

#### Scenario: Capabilities readable before connect
- **WHEN** a caller inspects an adapter's capabilities
- **THEN** the adapter reports its supported keys and its text and power-on support flags
