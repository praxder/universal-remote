...
from .androidtv_text import AndroidTvText, install_text
...
class AndroidTvSession(BaseSession):
    ...
    Text goes over the Remote v2 input-method path in `androidtv_text`, which needs no
    developer mode, ADB, or wireless debugging; keys go straight over the library.
    """
...
class AndroidTvAdapter:
    ...
    async def connect(self, device: "Device") -> AndroidTvSession:
        ...
        # Only now does the library have a protocol object for the text path to tap.
        return AndroidTvSession(
            remote, _CAPABILITIES, cert_dir, text=install_text(remote)
        )
