...
from tests.fakes import FakeAndroidTvRemoteFactory, ime_show_request
...
class TestAndroidTvText:
    def test_given_a_focused_field_when_text_is_sent_then_it_goes_over_remote_v2(self):
        factory = FakeAndroidTvRemoteFactory()
        adapter = AndroidTvAdapter(remote_factory=factory)
        credential = _paired_credential()
        ...
        async def scenario():
            session = await adapter.connect(_device(credential=credential))
            protocol = factory.remotes[-1]._remote_message_protocol
        ...
        edit = protocol.sent[-1].remote_ime_batch_edit
        assert edit.edit_info[0].text_field_status.value == "hello"
        assert edit.field_counter == 41
    ...
    def test_given_no_focused_field_when_text_is_sent_then_text_unsupported_is_reported(
        ...
        async def scenario():
            ...
            with pytest.raises(TextUnsupportedError):
                await session.send_text("hello")
            return factory.remotes[-1]._remote_message_protocol
    ...
    def test_given_text_send_fails_when_sending_then_text_unsupported_is_reported(self):
        ...
        async def scenario():
            ...
            protocol = factory.remotes[-1]._remote_message_protocol
            protocol.receive(ime_show_request(counter=41, value=""))
            protocol.send_error = RuntimeError("transport gone")
            with pytest.raises(TextUnsupportedError):
                await session.send_text("hello")
