...
class TestSelectDiscovered:
    ...
    def test_given_a_discovered_android_tv_when_added_then_no_extra_hint_is_shown(
        self, tmp_path
    ):
        # Android TV text needs no setup, so adding one lands on the device list with
        # the same toast as every other type rather than a modal.
        store = DeviceStore(path=tmp_path / "d.json")
        registry = _registry(
            FakeDiscoverAdapter(
                platform="androidtv",
        ...
        async def scenario():
            ...
            async with app.run_test() as pilot:
                ...
                assert 'Added "Living Room".' in messages
